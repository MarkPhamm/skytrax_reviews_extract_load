...
class AllAirlineReviewScraper:
    ...
    def __init__(
        ...
        output_path: str = "/opt/airflow/data/raw_data.csv"
    ):
...
if __name__ == "__main__":
    ...
    is_airflow = (
        ...
        os.path.exists('/opt/airflow') or
        'AIRFLOW_HOME' in os.environ
    )
    
    if is_airflow:
        # Running in Airflow/Docker environment
        airflow_output_path = "/opt/airflow/data/raw_data.csv"
        scraper = AllAirlineReviewScraper(
            num_pages_per_airline=50, 
            # max_airlines=30,  # Remove this for all airlines
