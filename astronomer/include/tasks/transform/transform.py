...
class AirlineReviewCleaner:
    ...
    def __init__(
        self,
        input_path: str = "/usr/local/airflow/include/data/raw_data.csv",
        output_path: str = "/usr/local/airflow/include/data/clean_data.csv",
    ):
    ...
    def rename_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df.rename(
            columns={"date": "date_submitted", "country": "nationality"}, inplace=True
        )
    ...
    def clean_nationality_column(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        df["nationality"] = (
            df["nationality"].str.replace(r"[()]", "", regex=True).str.strip()
        )
    ...
    def create_verify_column(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        verify_status = df["review_body"].str.contains(
            "trip verified", case=False, na=False
        )
        ...
        logger.info(
            f"Successfully created verify column with {verify_status.sum()} verified reviews"
        )
        return df
    ...
    def clean_recommended_column(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Successfully converted recommended column to boolean with {df['recommended'].sum()} positive recommendations"
        )
        return df
        ...
        def extract_city_airport(location: str) -> Tuple[Optional[str], Optional[str]]:
            ...
            if iata_match:
                ...
                if code in [v for v in AIRPORT_CODES.values()]:
                    ...
                    location_without_code = re.sub(
                        r"\b" + iata_match.group(1) + r"\b", "", location
                    ).strip()
            ...
            for city_name, airport_code in CITY_TO_AIRPORT.items():
                if (
                    city_name in location and len(city_name) > 3
                ):  # Exclude 3-letter airport codes
    ...
    def clean_route_column(
        self, df: pd.DataFrame, route_column: str = "route"
    ) -> pd.DataFrame:
        """
        ...
        df["destination_airport"] = route_components.apply(
            lambda x: x["destination_airport"]
        )
    ...
    def clean_aircraft_column(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def clean_entry(entry):
            ...
            entry = re.sub(
                r"\bEmbraer[- ]?(\d{3})\b", r"Embraer \1", entry, flags=re.IGNORECASE
            )
            entry = re.sub(
                r"\bEmbraerE(\d{3})\b", r"Embraer \1", entry, flags=re.IGNORECASE
            )
            entry = re.sub(
                r"\bEmbraer(\d{3})\b", r"Embraer \1", entry, flags=re.IGNORECASE
            )
            ...
            embraer_match = re.match(
                r".*(Embraer\s(170|190|195)).*", entry, flags=re.IGNORECASE
            )
            if embraer_match:
            ...
            boeing_short_match = re.match(
                r".*\b(B744|B747|B757|B767|B777|B787|B789|B737)\b.*",
                entry,
                flags=re.IGNORECASE,
            )
            ...
            # Match SAAB 2000
            saab_match = re.match(
                r".*(Saab\s2000|SAAB\s2000).*", entry, flags=re.IGNORECASE
            )
            if saab_match:
                return "Saab 2000"
